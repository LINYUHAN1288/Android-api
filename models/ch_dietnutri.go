...
type ChDietnutri struct {
	...
	DietID          string  `orm:"column(DietID);size(120);null"`
	...
	Brand           string  `orm:"column(Brand);size(255);null"`
	...
	Isvideo         int     `orm:"column(isvideo);null" description:"是否有视频数据"`
	IsTraning       int     `orm:"column(isTraning);null" description:"是否包含在深度训练模型中,1-包含,0-不包含"`
}

func (t *ChDietnutri) TableName() string {
	return "ch_dietnutri"
}

func init() {
	orm.RegisterModel(new(ChDietnutri))
}

// AddChDietnutri insert a new ChDietnutri into database and returns
// last inserted Id on success.
func AddChDietnutri(m *ChDietnutri) (id int64, err error) {
	o := orm.NewOrm()
	id, err = o.Insert(m)
	return
}

// GetChDietnutriById retrieves ChDietnutri by Id. Returns error if
// Id doesn't exist
func GetChDietnutriById(id int) (v *ChDietnutri, err error) {
	o := orm.NewOrm()
	v = &ChDietnutri{Id: id}
...
// GetAllChDietnutri retrieves all ChDietnutri matches certain condition. Returns empty list if
// no records exist
func GetAllChDietnutri(query map[string]string, fields []string, sortby []string, order []string,
	offset int64, limit int64) (ml []interface{}, err error) {
	o := orm.NewOrm()
	qs := o.QueryTable(new(ChDietnutri))
	// query k=v
	for k, v := range query {
...
// UpdateChDietnutri updates ChDietnutri by Id and returns error if
// the record to be updated doesn't exist
func UpdateChDietnutriById(m *ChDietnutri) (err error) {
	o := orm.NewOrm()
	v := ChDietnutri{Id: m.Id}
...
// DeleteChDietnutri deletes ChDietnutri by Id and returns error if
// the record to be deleted doesn't exist
func DeleteChDietnutri(id int) (err error) {
	o := orm.NewOrm()
	v := ChDietnutri{Id: id}
	// ascertain id exists in the database
	if err = o.Read(&v); err == nil {
		var num int64
		if num, err = o.Delete(&ChDietnutri{Id: id}); err == nil {
